...
@torch.no_grad()
def generate_pseudolabels_batch(patch_tokens, targets, config, text_model, tokenizer):
    """
    Generate pseudolabels from precomputed patch token embeddings.
    ...
    Args:
        ...
        text_model: Text encoder model (for build_text_embeddings)
    ...
    # Build text embeddings for ALL foreground classes + background classes (once for entire batch)
    all_class_names = ALL_FG_CLASS_NAMES + BACKGROUND_CLASS_NAMES
    text_emb_all = build_text_embeddings(text_model, tokenizer, all_class_names)  # [num_all_classes, D]
    num_all_fg = len(ALL_FG_CLASS_NAMES)
    num_bg = len(BACKGROUND_CLASS_NAMES)
